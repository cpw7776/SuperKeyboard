...
- **Device:** AVD `Medium_Phone_API_36.1` (fallback `Pixel_9_Pro_Fold`); emulator binary at `~/Library/Android/sdk/emulator/emulator`. No physical device attached.
...
| S4 | On-device sanity | Sideload APK; enable IME in Android Settings; type keys; copy text → check clipboard history | Keyboard renders & types; clipboard entry appears | ~3 min manual |
