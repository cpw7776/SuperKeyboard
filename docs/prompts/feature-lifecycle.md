...
> **Gate count is stack-dependent — five with a standalone automated test suite, four without.** Gate #2 (the Test-Suite Summary) presumes the project *has* a standalone automated suite (unit/integration). A project that legitimately has none — where the Phase 4.2 FULL SUITE GATE is a build / compile / typecheck / sideload pipeline rather than a test runner (native-mobile sideload, a CLI build, a data-app smoke run) — runs **four** gates: Retrospective + Code Quality + Context Docs + Documentation. There is **no Test-Suite gate block** in the final paste — the test-suite *gate* is genuinely absent, not a fifth slot left blank. What the missing runner would have carried doesn't vanish; it **folds** to wherever the project designates (record the choice in `docs/KIT_DEVIATIONS.md`): the default landing is the **Retrospective Gate's Gap A** (the build/sideload result + any suite-level lesson), but a project MAY instead keep a project-named build/sideload gate block in the ordered list if it prefers a standalone proof. Either way the build/sideload proof is still produced and pasted *somewhere* verbatim — a fold, not a skip. Do **not** scaffold an empty suite just to reach five, and do **not** drop the proof to justify four. (The kit's stamped default is "five"; "four" is the sanctioned reduction for suite-less stacks — set the machine-readable `Gate count: four (suite-less)` line in `docs/KIT_DEVIATIONS.md` so upgrades branch on it deterministically and never mistake a *correct* "four" for the historical v5.14 "four-gates" wording bug.)
