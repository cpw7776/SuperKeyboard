...
> **Model:** Switch the chat to **`opus`** before running this prompt. Debugging is pure reasoning — three-hypothesis generation, elimination strategy, root-cause synthesis. This is exactly the kind of higher-order thinking the kit reserves for Opus. The post-diagnosis fix is short enough to stay in the same chat; don't switch mid-flow. Same model split as `feature-lifecycle.md` — Opus on the critical path, Sonnet only on structured-execution sub-agents; see that prompt's model split table.
