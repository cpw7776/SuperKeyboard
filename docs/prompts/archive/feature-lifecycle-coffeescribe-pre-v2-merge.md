...
- `/supabase-postgres-best-practices` — Database changes
...
- Read all relevant context: `docs/context/Context_Index_File.md`, `PRODUCTION_READY_2.0.md`, `database_reference_guide.md`, `API_REFERENCE.md`
...
### 2.7 Skills Map
Identify which skills apply at each PRD step (e.g., `/supabase-postgres-best-practices` for DB tasks, `/tdd` for tests).
...
1. Run `npm run test:run` — capture current test results
...
### 3.0 Resource Management (CRITICAL — 16GB MacBook Air, No Fan)

**This machine overheats easily. Follow these rules throughout ALL phases:**

- **Max 2 parallel agents at a time.** Never launch more than 2 simultaneously.
- **Kill processes AS SOON AS they finish.** After every test run, build, TypeScript check, or agent completion, immediately check and kill orphaned processes: `ps aux | grep node | grep -v grep | grep -v VSCodium | grep -v Claude`. MCP servers (Playwright, Apify), dev servers, and test runners must NEVER be left running.
- **Never leave processes running.** This is non-negotiable. Kill them the moment they are done.
...
- After every major component/feature addition: run `npm run test:run` to catch regressions early
...
The testing-agent lives at `.claude/agents/testing-agent.md` and has mandatory RAM-hygiene pre-flight + teardown for this 16GB machine. Running agent-browser inline bypasses these safeguards and has historically left multi-GB of orphaned MCP/chromium processes.

**Invocation:**

1. Generate (or reuse) the feature-specific test plan at `docs/testing-agents/{feature-name}-tests.md` via the `create-testing-agent.md` prompt.
2. Start the dev server on a known port: `npm run dev` (localhost:3000 by default; use a different port per worktree).
...
4. Invoke the testing-agent with this input block:
   ...
   - Tiers to test (Admin, Creator, Pro, Free as applicable)
...
This is the gate the targeted-files-only test runs CANNOT replace. See `CLAUDE.md` → "Test-suite gate" for the full rule.

1. **Pre-flight cleanup.** Run `ps aux | grep -E "node|next dev|chrom|playwright" | grep -v grep | grep -v VSCodium | grep -v Claude` and kill any leftover dev servers, MCP servers, or test runners before launching the suite. Mac is 16GB no-fan; one heavy operation at a time.
2. **Run `npm run test:run` (full suite, no path argument).** Capture the verbatim final summary lines from the output:
   - `Test Files  X passed | Y skipped (Z)`
   - `Tests  A passed | B skipped | C todo (D)`
   **Paste BOTH lines into the transcript.** "All green" or "tests pass" without these two lines does not satisfy the gate. Sub-agent reports that say "tests pass" without the lines also do not satisfy it — the main agent must run the suite and paste the proof.
3. **Run `npm run build`.** Confirm `Tasks: N successful, N total` and `exit code 0`.
4. **Failure handling:** if any test fails, fix the code (not the test, unless test logic is wrong), re-run, re-paste. Pre-existing failures unrelated to the feature must be either fixed in a separate pre-flight commit on the branch, or escalated to the user for explicit approval to log them in `docs/known-test-failures.md` before continuing — they are NOT a "not my problem" exemption.
5. **Skip-count drift handling:** if `B` (skipped tests) is higher than the baseline in `docs/known-test-skips.md`, document the new skips in the same file with a reason, or remove them. The merge commit message must mention any deliberate skip additions.
...
### 4.4 Test Summary
- Browser test result: passed / failed / fixed
...
4. Re-run `npm run test:run` and `npm run build` after fixes.
...
- **What changed and why** — for each deviation: what did the original plan say, what actually happened, and what caused the change? (e.g., "planned to add a new table but discovered existing table already had the right columns", "API shape changed because the component needed different data than anticipated")
...
After manual testing and any resulting fixes, rewrite these documents so that if someone reads them tomorrow, they accurately describe what was built and how it works:
...
- **ADR:** Update technical approach, data flow, and schema to match the actual implementation. If architectural decisions changed during development, rewrite the justification to reflect the real decision, not the planned one.
...
Run `/technical-writing` to ensure quality. This is not box-ticking — it's a rewrite pass.

### 5.3.5 Re-run the full test suite (mandatory pre-merge)

Even if Phase 4.2 was clean, re-run `npm run test:run` after **any** post-manual-testing fix and after the PRD/ARD/Architecture rewrites in 5.3 (in case anything regressed). Paste the verbatim summary lines into the transcript again — this is the line that will sit in the merge commit's review trail. Do not skip this on the basis that "I only edited docs in 5.3"; the gate is procedural, not contingent on what changed.

Re-run `npm run build` too. Both must be clean before 5.4.

### 5.4 Code Quality & Security — MANDATORY (delegated to code-quality-agent)
...
### 5.6 Documentation — MANDATORY (delegated to two sub-agents, sequential)

**Launch these two sub-agents sequentially (NOT parallel — 16GB RAM budget). Each produces a verbatim gate block that must appear in the main transcript.**

#### 5.6a — context-docs-agent
...
**Required output:** Paste the agent's verbatim `CONTEXT DOCS GATE:` block (8 numbered items) into the main transcript.

#### 5.6b — docs-auditor-agent
...
**Required output:** Paste the agent's verbatim `DOCUMENTATION GATE (User-Facing):` block (5 numbered sections, every decision-tree question answered) into the main transcript.

### 5.7 Gate Output Required Rule — cannot proceed to git until all four gates are pasted

Before Phase 5.8 (Git), the main transcript must contain, in order:
1. The verbatim **Test-Suite Summary lines** from the Phase 5.3.5 re-run (`Test Files  X passed | Y skipped (Z)` and `Tests  A passed | B skipped | C todo (D)`) — both lines, with `B passed` or `0 failed` ratio matching `docs/known-test-failures.md` (i.e. zero unexplained failures).
...
3. The verbatim `CONTEXT DOCS GATE:` block from context-docs-agent (8 items).
4. The verbatim `DOCUMENTATION GATE (User-Facing):` block from docs-auditor-agent (5 sections with all decision-tree answers).

**Enforcement:** If any gate block is missing, truncated, paraphrased, or shows an unresolved failure, Phase 5 is NOT complete. You must re-invoke the relevant sub-agent (or re-run the full suite for the test gate) and escalate to the user if a real failure surfaces. Do NOT commit, do NOT merge, do NOT declare the feature done.

"I ran the checks and everything was fine" is NOT acceptable. The gate blocks (and verbatim test summary lines) are the proof. No blocks = no completion.
...
### 5.9 Next Steps
- Review `Epics_and_User_Stories.md`, identify next logical step with 2-3 options
...
Phase 4 (browser testing) and Phase 5 (code quality + documentation) now run via dedicated sub-agents (`testing-agent`, `code-quality-agent`, `context-docs-agent`, `docs-auditor-agent`). Each sub-agent produces a verbatim gate block; progression through Phase 4 and Phase 5 is gated on those blocks appearing in the main transcript. No new human stop points are introduced — the three stops below still cover the only moments the AI pauses for the user.
