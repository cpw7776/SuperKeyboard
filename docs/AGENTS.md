...
1. **`RETROSPECTIVE GATE:`** — verbatim from Phase 5.1 (main agent). The unconditional retro sweep: `Findings swept: N` over every manual-test finding ∪ `fix(` commit, with Gap A (test suite) / Gap B (testing agent) / **Gap C (implementation/code lessons → `Implementation_Patterns.md`)** / **Gap D (context-fit epic sweep — kit v5.19+, `.claude/skills/context-fit/`: clean | cards | ⚠ INCONCLUSIVE | skill absent)** / planning / Tier-3 / deliberately-not-lessoned lines populated. A zero-finding epic still prints it with all lines `none`.
...
- **`context-fit`** (v5.19+) — Gap D's engine: a **read-only** scan of this project's own session transcripts (main agent + sub-agents + parallel panes) that finds domain subsystems agents reconstruct by hand (concentrated re-reads) with no owning `docs/context/` doc, and proposes context docs / scoped indexes through a strict Necessity Gauntlet (**default No; accept = write + register in `context-docs-agent`'s target-files**). Auto-invoked by `feature-lifecycle.md` Phase 5.1 (Gap D) at epic close; lightweight one-question checks in `bugfix.md` 2.4 and `reconcile-change.md` 2.1 feed it via the findings channel (`~/.claude/context-fit-findings.md`). Unlike `cmux-orchestrator` there is no user-level canonical copy — the in-repo skill IS the implementation (same ensure-present, never-clobber rule on upgrade).
